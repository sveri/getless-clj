...
(defn search-page [{:keys [off-url off-user off-password]} search only-one-locale {:keys [localize]}]
  (let [only-one-locale? (= "on" only-one-locale)
        products (if-not (s/blank? search)
                   ...
                   {})
        products (off/sanitize (:products products))
        products (mapv #(off/add-nutriments % localize off/nutriments-to-extract)
                   products)]
    (layout/render "off/search.html" {:products products :search search})))
